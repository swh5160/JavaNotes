...
Ajax: 全称**Asynchronous JavaScript And XML**，异步的**JavaScript和XML**。其作用有如下2点：
...
针对于上述Ajax的局部刷新功能是因为Ajax请求是异步的，与之对应的有同步请求。接下来我们==介绍一下异步请求和同步请求的区别。==
...
- 客户端
  ...
  第二步：创建==XMLHttpRequest对象，用于和服务器交换数据，也是原生Ajax请求的核心对象，==提供了各种方法。代码如下：
  ...
  第四步：我们通过==绑定事件的方式，来获取服务器响应的数据。（函数称之为回调、监听、钩子函数）==
...
![image-20230316160121415](assets/image-20230316160121415.png)
...
我们观察发现，index.html的代码很简洁，但是浏览器所呈现的index.html内容却很丰富，代码和内容不匹配，所以vue是如何做到的呢？接下来我们学习一下vue项目的启动及运行流程。

基本流程

1.启动: index.html被main.js读取并加载

2.加载: 创建Vue对象，并加载所导入的组件对象

3.渲染: 每个组件对象由三部分构成：template（模板） 、script（脚本）、style（样式）

4.运行：组件对象的三部分，保存当前项目（自动编译）即可看到运行效果
...
![image-20230225162133338](assets/image-20230225162133338.png)
...
补充：定义代码片段（了解）
 ...
 可以把上面的定义为代码模板，需要时输入关键字（!vue）就能出来代码结构。

1. 创建代码片段的代码模板文件  打开vscode，使用快捷键打开搜索面板

   ![image-20230225153524507](assets/image-20230225153524507.png)

2.  使用https://snippet-generator.app/产生代码模板，然后复制第1步最后需要填写的json文件中

   ![image-20230225161943602](assets/image-20230225161943602.png)

3. 删除代码配置

   ![image-20230225162029978](assets/image-20230225162029978.png)
...
首先我们来到VS Code中，在views目录下创建 tlias/EmpView.vue这个vue组件，并且编写组件的基本模板代码
...
![image-20230309155430632](assets/image-20230309155430632.png) 
...
![image-20230225185153352](assets/image-20230225185153352.png)
...
1. 安装axios: npm install axios （在项目里面执行）
...
然后**重启项目**，来到我们的EmpView.vue组件页面，在<script>标签里面import命令导入axios，代码如下：

<img src="assets/image-20230225185927393.png" alt="image-20230225185927393" style="zoom:80%;" />
...
将代码/DeptView.vue拷贝到我们当前EmpView.vue同级，其结构如下：
...
1. 安装: vue-router插件（已经安装过）
2. 定义: 在router/index.js（路由表）中定义路由配置
3. 引入: 在main.js中（全局入口），引入了router功能
4. 使用: 在App.vue中（主页）中，使用路由组件<router-view>（去掉之前的组件）
5. 导航: 在EmpView.vue和DeptView.vue（导航页）中，使用就是<router-link>;
...
我们直接通过VS Code的NPM脚本中的 ”**build vue-cli-service build**“ 提供的build按钮来完整，如下图所示，直接点击即可：
