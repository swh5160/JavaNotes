...
![image-20230319093246222](assets/image-20230319093246222.png)
...
其实呢，在SpringBoot进行web程序开发时，它**内置了一个核心的Servlet程序 DispatcherServle**t，称之为前端控制器。 DispatcherServlet 负责**接收**页面发送的请求，然后根据执行的规则，将请求再转发给后面的请求处理器Controller，请求处理器处理完请求之后，最终再由DispatcherServlet给浏览器响应数据。

![image-20220826165340157](assets/image-20220826165340157.png)

那将来浏览器发送请求，会携带请求数据，包括：请求行、请求头；请求到达tomcat之后，tomcat会负责解析这些请求数据，解析后的请求数据会传递给Servlet程序的HttpServletRequest对象，**==那也就意味着 HttpServletRequest 对象就可以获取到请求数据==**。 而Tomcat，还给Servlet程序传递了一个参数 HttpServletResponse，通过这个对象，我们就可以给浏览器设置响应数据 。
...
![image-20230319092446655](assets/image-20230319092446655.png)
...
##### 补充：为什么controller层集合作为参数时，必须有@RequestParam注解？

答：==但需要在list前面添加@RequestParam，只有增加了这个注解后，Spring MVC才知道原来请求中包含的复合数据，被转化为list这种形式进行存储。==

![image-20230319131730705](assets/image-20230319131730705.png)

#### 1.4.3 集合（Map之复合数据类型）

> 他返回值是以键值对的形式，但是他有个缺陷，如果表单中存在复合数据，Map是无法接收的。注意：仍然需要在Map中加上@RequestParam

```java
 @GetMapping("/mapParamDate")
    public String mapParamDate(@RequestParam Map<String,String> map){
        map.forEach((key, value) -> System.out.println(key + ":" + value));
        return "成功";
    }
```

![image-20230319132532463](assets/image-20230319132532463.png)
...
![image-20230319121117988](assets/image-20230319121117988.png)
...
```java
 //路径参数
    @PostMapping("/postParamPath/name/{name}/age/{age}")
    public String postParamPath(@PathVariable String name,@PathVariable Integer age){
        System.out.println(name + ": " + age);
        return "postParamPath请求成功";
    }
```

![image-20230319153216443](assets/image-20230319153216443.png)

### 1.8 表单参数

数据存在表单中以key-value存放（表单分为两种默认和文件）

![image-20230319152936931](assets/image-20230319152936931.png)

```java
 /**
     * post请求不传递对象，传递对象中的部分属性
     * @param name
     * @param age
     * @return
     */ 
@PostMapping("/postParamJson")
    public String postParamJson(String name,Integer age){
        System.out.println(name + ": " + age);
        return "postJson请求成功";
    }
```
...
Springboot项目的静态资源(html，css，js等前端资源)默认存放目录为：classpath:/static 、 classpath:/public
...
在我们进行程序设计以及程序开发时，尽可能让**每一个接口、类、方法的职责更单一些（单一职责原则）。**
...
##### **==面试题：什么是IOC，什么又是DI，他们有什么区别？==**

​		**IOC顾名思义**就是控制饭转，控制：对象的创建的控制权。反转：将对象的控制权交给spring容器。之前我们都是通过new创建对象，现在直接在spring容器中取，从而达到对象之间的依赖关系，降低对象之间的耦合度。

​		**DI;依赖注入即实现方式，**意思自身对象中的内置对象是通过注入的方式进行创建。实现方式有三种构造方法、setter注入、注解注入。
...
> 入门程序举例：在EmpController运行的时候，就要到IOC容器当中去查找EmpService这个类型的对象，而我们的IOC容器中刚好**有一个**EmpService这个类型的对象，所以就找到了这个类型的对象完成注入操作。
...
使用@Primary注解：当存在多个相同类型的Bean注入时，加上@Primary注解，来**确定默认**的实现。
...
#### ==3.3.4 面试题@Autowird 与 @Resource的区别==
...
> - 注入默认方式
>
>   - @Autowired 默认是按照类型注入；默认情况下它要求依赖对象必须存在（可以设置它required 属性为false）。
>
>   - 而@Resource是按照名称注入；只有当找不到与名称匹配的bean 才会按照类型来装配注入。
>
> - @Autowired 可用于：构造函数、成员变量、Setter 方法
>
> 

#### ==3.3.5 面试题DI依赖注入的几种方式==

![image-20230319182530135](assets/image-20230319182530135.png)

#### ==3.3.6 IOC和DI是什么?==

**IOC顾名思义就是控制饭转**，控制：对象的创建的控制权。反转：将对象的控制权交给spring容器。之前我们都是通过new创建对象，现在直接在spring容器中取，从而达到对象之间的依赖关系，降低对象之间的耦合度。

**DI;**依赖注入即实现方式，全称为Dependency Injection，意思自身对象中的内置对象是通过注入的方式进行创建。
